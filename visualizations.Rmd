...
mtb_diff <- read_excel("fran_mtb_val.xlsx")
gen_diff <- read_excel("fran_genera_diff_vis.xlsx")
```
#Some data formatting:
```{r}
gen_diff <- as.data.frame(gen_diff)
rownames(gen_diff) <- gen_diff[,1]
#Extract Disease Group:
`Disease Group` <- gen_diff$Disease.Group
#Remove the sample and disease group columns:
gen_diff <- gen_diff[,-c(1,76)]

#Make samples the row names and remove that column:
mtb_diff <- as.data.frame(mtb_diff)
rownames(mtb_diff) <- mtb_diff[,1]
mtb_diff <- mtb_diff[,-1]
```
...
```
#Heatmaps of selected microbes and metabolites:

Format the required data:
```{r}
#Create a Dataframe of the disease group labels to add as an annotation on the heatmap:
dis_stat_df <- as.data.frame(`Disease Group`)
rownames(dis_stat_df) <- rownames(gen_diff)
```

This is a function to allow easy saving of the heatmap figure as a pdf:
```{r}
...
Microbe Heatmap:
```{r}
#PHeatmap requires the data to be in a matrix:
gen_diff_mat <- data.matrix(gen_diff)
#Log the values as used in the model:
log_gen_diff <- log(gen_diff_mat+0.0001)

#Create Heatmap:
ph <- pheatmap(log_gen_diff, 
               annotation_row = dis_stat_df, #Show disease group of each sample
               cellwidth = 10, cellheight = 10, #change the cell size so the text 
               #does not overlap
               #increase the legend text size, while the other remains the same:
               fontsize_row = 10, fontsize_col = 10, fontsize = 15, 
               #alter the numbers on the legend:
               legend_breaks = c(-9, -7, -5, -3, -1, 0))

#Save Heatmap as a pdf:
#save_pheatmap_pdf(ph, "micro_heat.pdf", width = 17, height =33)
```

Metabolite Heatmap:
```{r}
#Rename column for easier viewing:
names(mtb_diff)[12] <- 'Tetrahydro-carboxylic acid*'
#Make a matrix:
met_diff_mat <- data.matrix(mtb_diff)
#Log as input in the model:
log_met_diff <- log(mtb_diff+1)

#Create Heatmap:
ph_mtb <- pheatmap(log_met_diff, 
                   annotation_row = dis_stat_df, 
                   cellwidth = 10, cellheight = 10, 
                   fontsize = 13, fontsize_row = 10, fontsize_col = 10)

#Save heatmap as pdf:
#save_pheatmap_pdf(ph_mtb, "mtb_heat.pdf", width = 10, height = 35)
```
#Correlation Diagrams:
Data Formatting:
```{r}
#extract the 9 overlapping key microbe features only:
imp_gen <- gen_diff[,c('CAJJNI01','CAJOIG01','Flavonifractor','JAGTTR01','RGIG426','RUG762','UBA11774','UBA1774','Ventricola')]

#combine the two dataframes:
both_fet <-cbind(imp_gen, mtb_diff)
```

Graph Diagram:
```{r}
pdf("qgraph_plot.pdf", width = 8, height = 6) #create pdf to save diagram
#create graph diagram:
qgraph(cor(both_fet),
       layout = "spring", #organisational choice
       legend = F,
       graph = "pcor") #use partial correlation
dev.off()
```

Correlation Grid:
```{r}
#create diagram to show correlation between each microbe and metabolite
ggcorrplot(cor(both_fet), legend.title = "Correlation", hc.order = TRUE) #cluster the microbes and metabolites and order them based on this

#Save the diagram:
#ggsave(file="corr_map.pdf", height = 5, width =5, dpi=300)
```
